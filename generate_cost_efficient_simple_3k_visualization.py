#!/usr/bin/env python3
"""
Cost-Efficient Strategy Training Results Visualization (3000 Episodes) - Simple Layout
...
def create_cost_efficient_simple_visualization():
    """Create simple 4-subplot visualization matching the 1000-episode layout"""
    
    try:
        # Load training data
        data_path = "outputs_pump_cbm_v047_cost_efficient/training_history.json"
        ...
        print(f"[SUCCESS] Loaded Cost-Efficient training data: {len(rewards)} episodes")
        
        # Create figure with 2x2 subplots
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 10))
        fig.suptitle('Cost-Efficient Strategy - Training Results (3000 Episodes)', fontsize=16, fontweight='bold')
        ...
        ax1.plot(rewards, alpha=0.3, color='lightgreen', linewidth=0.5)
        ax1.plot(range(window-1, len(rewards)), reward_ma, color='darkgreen', linewidth=2, label=f'Moving Average ({window}ep)')
        ...
        ax2.plot(costs, alpha=0.3, color='lightcyan', linewidth=0.5)
        ax2.plot(range(window-1, len(costs)), cost_ma, color='darkcyan', linewidth=2, label=f'Moving Average ({window}ep)')
        ...
        ax3.hist(final_100_rewards, bins=15, alpha=0.7, color='lightgreen', edgecolor='darkgreen', linewidth=1)
        ...
        ax4.plot(range(stability_window-1, len(rewards)), rolling_std, color='darkgreen', linewidth=2)
        ...
        # Save visualization
        output_dir = Path("run_3K_episodes/outputs_pump_cbm_v047_cost_efficient")
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / "cost_efficient_simple_training_results.png"
...
if __name__ == "__main__":
    print("[INFO] Generating Cost-Efficient Simple 3000-Episode Training Results Visualization...")
    success = create_cost_efficient_simple_visualization()
    if success:
